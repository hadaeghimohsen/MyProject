...
{
   partial class ADM_MBCO_F : ISendRequest
   {
      ...
      private void ProcessCmdKey(Job job)
      {
         ...
         {            
            job.Next =
               new Job(SendType.SelfToUserInterface, "ADM_MBCO_F", 04 /* Execute UnPaint */);
         }
      ...
      private void Actn_CalF_P(Job job)
      {
         try
         {
            ...
            {
               if ((job.Input as XElement).Attribute("type").Value == "renewcontract")
               {
            ...
            {
               if ((job.Input as XElement).Attribute("type").Value == "refresh" || (job.Input as XElement).Attribute("type").Value == "fighter")
                  Execute_Query();
            }
            else
               Execute_Query();
