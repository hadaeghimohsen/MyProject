...
{
   public partial class MCC_RQST_F : UserControl
   {
      public MCC_RQST_F()
      {
         InitializeComponent();
      ...
      private void Tsb_SaveItem_Click(object sender, EventArgs e)
      {
         ...
         {            
            ...
            Func<Data.Request_Row, XElement> INS_FPTS_U = new
               ...
               {
                  if (r.RWNO > 0)
                     return new XElement("Member_Ship",
                           new XAttribute("strtdate", r.Member_Ships.SingleOrDefault(m => m.RECT_CODE == "001").STRT_DATE),
                           new XAttribute("enddate", r.Member_Ships.SingleOrDefault(m => m.RECT_CODE == "001").END_DATE)
                       );
                  else
                     return new XElement("Member_Ship",
