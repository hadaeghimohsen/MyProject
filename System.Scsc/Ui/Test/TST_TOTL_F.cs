...
{
   public partial class TST_TOTL_F : UserControl
   {
      public TST_TOTL_F()
      {
         InitializeComponent();
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         ...
         OldRecdBs1.DataSource = iScsc.Tests.Where(c => c.FIGH_FILE_NO == Rqro.FIGH_FILE_NO && c.RECT_CODE == "004" && c.Fighter.TEST_RWNO_DNRM == c.RWNO);
         OldFPBs1.DataSource = iScsc.Fighter_Publics.Where(fp => fp.FIGH_FILE_NO == Rqro.FIGH_FILE_NO && fp.RECT_CODE == "004" && fp.Fighter.FGPB_RWNO_DNRM == fp.RWNO);
      }
               ...
               new XElement("Process",
                  new XElement("Request",
                     ...
                     new XElement("Request_Row",
                           ...
                           new XElement("Test",
                              new XAttribute("crtfdate", CRTF_DATE_DateTime.Value == null ? "" : CRTF_DATE_DateTime.Value.Value.ToString("yyyy-MM-dd")),
                              new XAttribute("crtfnumb", CRTF_NUMB_TextEdit.EditValue ?? ""),
                              new XAttribute("testdate", TEST_DATE_DateTime.Value == null ? "" : TEST_DATE_DateTime.Value.Value.ToString("yyyy-MM-dd")),
                              new XAttribute("globcode", gLOB_CODETextEdit.EditValue ?? ""),
                              new XAttribute("mtodcode", Mtod_LookUpEdit.EditValue ?? ""),
                              new XAttribute("ctgycode", Ctgy_LookUpEdit.EditValue ?? ""),
                              new XAttribute("rslt", RSLT_LookUpEdit.EditValue ?? "")
                        )
                  )
               )
            ));
      ...
      private void Mtod_LookUpEdit_EditValueChanged(object sender, EventArgs e)
      {
         try
         {
      ...
      private void FILE_NO_LookUpEdit_EditValueChanged(object sender, EventArgs e)
      {
         var figh = FighsBs1.Current as Data.Fighter;
         if (figh == null) return;

         MtodBs1.DataSource = iScsc.Methods.Where(m => m.CODE == figh.MTOD_CODE_DNRM);
      }
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "006")))}
                  })
               );
         } 
      }
