         ...
         switch (xinput.Attribute("type").Value)
         {
            case "changeinfo":
               if (RqstBs1.Count > 0 && (RqstBs1.Current as Data.Request).RQID > 0)
                  RqstBs1.AddNew();
               ServFileNo_Lov.EditValue = Convert.ToInt64(xinput.Attribute("fileno").Value);
               if (xinput.Attribute("auto").Value == "true")
               {
                  RqstBnARqt1_Click(null, null);

                  var figh = iCRM.Services.First(f => f.FILE_NO == Convert.ToInt64(xinput.Attribute("fileno").Value));
