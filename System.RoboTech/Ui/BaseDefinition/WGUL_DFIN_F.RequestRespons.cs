...
{
   partial class WGUL_DFIN_F : ISendRequest
   {
               ...
               new List<Job>
               {
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        {
                           Input = new List<string> {"<Privilege>68</Privilege><Sub_Sys>12</Sub_Sys>", "DataGuard"},
                           AfterChangedOutput = new Action<object>((output) => {
                              if ((bool)output)
                                 return;
                              #region Show Error
                              job.Status = StatusType.Failed;
                              MessageBox.Show(this, "خطا - عدم دسترسی به ردیف 68 امنیتی", "خطا دسترسی");
                              #endregion                           
                           })
                        },
                        #endregion                        
      ...
      private void LoadData(Job job)
      {
         ...
         vComputerBs.DataSource = iRoboTech.V_Computers;
         DactpBs.DataSource = iRoboTech.D_ACTPs;
         DurltBs.DataSource = iRoboTech.D_URLTs;
