         ...
         try
         {
            ...
            if (_getFollowers.Succeeded || _getFollowers.Value.Count > 0)
            {
               foreach (var f in _getFollowers.Value)
               {
                  ...
                  {
                     ++s;
                       iRoboTech.ExecuteCommand(string.Format("INSERT INTO dbo.Robot_Instagram_Follow(Rins_Code, Code, Inst_Pkid, User_Name, Full_Name, Folw_Type) VALUES({0}, 0, {1}, {2}, {3}, '001');", inst.CODE, f.Pk, f.UserName, f.FullName));
            ...
            if (_getFollowing.Succeeded || _getFollowing.Value.Count > 0)
            {
               foreach (var f in _getFollowing.Value)
               {
                  ...
                  {
                     ++s;
                       iRoboTech.ExecuteCommand(string.Format("INSERT INTO dbo.Robot_Instagram_Follow(Rins_Code, Code, Inst_Pkid, User_Name, Full_Name, Folw_Type) VALUES({0}, 0, {1}, {2}, {3}, '002');", inst.CODE, f.Pk, f.UserName, f.FullName));
