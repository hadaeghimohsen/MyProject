...
{
   public partial class SHOW_ATRQ_F : UserControl
   {
      public SHOW_ATRQ_F()
      {
         InitializeComponent();
      ...
      {
         RqroBs1.DataSource = iScsc.Request_Rows.FirstOrDefault(rr => rr.RQST_RQID == Rqid && rr.FIGH_FILE_NO == FileNo);
      }
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         try
         {
            ...
            if (iScsc.Member_Ships.Any(mb => mb.RQRO_RQST_RQID == Rqid && mb.FIGH_FILE_NO == FileNo && mb.RECT_CODE == "004"))
               MbspBs1.DataSource = iScsc.Member_Ships.FirstOrDefault(mb => mb.RQRO_RQST_RQID == Rqid && mb.FIGH_FILE_NO == FileNo && mb.RECT_CODE == "004");
            else
               MbspBs1.DataSource = iScsc.Member_Ships.FirstOrDefault(mb => mb.RQRO_RQST_RQID == Rqid && mb.FIGH_FILE_NO == FileNo && mb.RECT_CODE == "001");
         }
         catch {
            Gb_Mbsp.Visible = false;
            STRT_DATE.Value = DateTime.Now.Date;
