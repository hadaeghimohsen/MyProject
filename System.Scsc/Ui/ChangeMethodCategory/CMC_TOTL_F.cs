...
{
   public partial class CMC_TOTL_F : UserControl
   {
      public CMC_TOTL_F()
      {
         InitializeComponent();
               ...
               new XElement("Process",
                  new XElement("Request",
                     ...
                     new XElement("Request_Row",
                           ...
                           new XElement("ChngMtodCtgy",
                              new XElement("Crtf_Date", CRTF_DATE_DateTime.Value == null ? "" : CRTF_DATE_DateTime.Value.Value.ToString("yyyy-MM-dd")),
                              new XElement("Crtf_Numb", CRTF_NUMB_TextEdit.EditValue ?? ""),
                              new XElement("Test_Date", TEST_DATE_DateTime.Value == null ? "" : TEST_DATE_DateTime.Value.Value.ToString("yyyy-MM-dd")),
                              new XElement("Glob_Code", gLOB_CODETextEdit.EditValue ?? ""),
                              new XElement("Mtod_Code", Mtod_LookUpEdit.EditValue ?? ""),
                              new XElement("Ctgy_Code", Ctgy_LookUpEdit.EditValue ?? "")
                        )
                  )
               )
            ));
      ...
      private void Mtod_LookUpEdit_EditValueChanged(object sender, EventArgs e)
      {
         try
         {
            if (Mtod_LookUpEdit.ItemIndex == -1) return;
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "011")))}
                  })
               );
         }
      }
