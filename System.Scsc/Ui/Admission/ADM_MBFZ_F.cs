...
{
   public partial class ADM_MBFZ_F : UserControl
   {
      public ADM_MBFZ_F()
      {
         InitializeComponent();
      ...
      private void Btn_RqstRqt3_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            if (!StrtDate_DateTime003.Value.HasValue) { StrtDate_DateTime003.Value = DateTime.Now; StrtDate_DateTime003.Focus(); }
            if (!EndDate_DateTime003.Value.HasValue) { EndDate_DateTime003.Value = DateTime.Now.AddDays(7); EndDate_DateTime003.Focus(); }
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "026")))}
                  })
               );
         }
      }
