...
using System.Dolphin.ExceptionHandlings;
...
namespace System.Dolphin.Ui.MasterPage
{
   partial class FRST_PAGE_F : ISendRequest
   {
      ...
      private Data.iDolphinDataContext iDolphin;
      ...
      private void Set(Job job)
      {
         ...
         iDolphin = new Data.iDolphinDataContext(GetConnectionString.Output.ToString());

         //Fga_Uclb_U = (iCRM.FGA_UCLB_U() ?? "").Split(',').Select(c => (long?)Int64.Parse(c)).ToList();
         //Lbs_CrntUser.Text = 
         CurrentUser = iDolphin.GET_CRNTUSER_U(new XElement("User", new XAttribute("actntype", "001")));
