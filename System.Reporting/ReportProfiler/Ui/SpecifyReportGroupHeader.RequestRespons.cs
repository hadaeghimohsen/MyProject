...
{
   partial class SpecifyReportGroupHeader : ISendRequest
   {
      ...
      private void ProcessCmdKey(Job job)
      {
         ...
         else if (keyData == Keys.Escape)
         {
            if (freeForm)
               job.Next =
                  new Job(SendType.SelfToUserInterface, "SpecifyReportGroupHeader", 04 /* Execute UnPaint */);
            else
      ...
      private new void Paint(Job job)
      {
         Job _Paint = new Job(SendType.External, "DefaultGateway",
            new List<Job>
            {
               ...
               new Job(SendType.SelfToUserInterface, "Wall", 15 /* Execute Push */) {  Input = new List<object> { "Reporting:ReportProfiler:SpecifyReportGroupHeader", this }  },
      ...
      {
         job.Next =
            new Job(SendType.SelfToUserInterface, "SpecifyReportGroupHeader", 08 /* LoadRolesOfUser */);
         job.Status = StatusType.Successful;
      }
         ...
         Job _LoadRoleOfUser =
            new Job(SendType.External, "DefaultGateway",
               new List<Job>
               {
                  ...
                  #endregion
                  new Job(SendType.SelfToUserInterface, "SpecifyReportGroupHeader", 10 /* Executive LoadActiveDatasource */)
               });
      ...
      /// </summary>
      /// <param name="job"></param>
      private void LoadGroupHeadersOfRole(Job job)
      {
         ...
         _DefaultGateway.Gateway(
            new Job(SendType.External, "Localhost", "Commons", 04 /* Execute DoWork4Odbc */, SendType.Self)
            {
               ...
               Input = new List<object>
               {
                  ...
                  true,
                  "xml",
                  string.Format(@"<Request type=""{0}""><Role>{1}</Role><GroupHeader>{2}</GroupHeader></Request>", job.Input,cb_roles.SelectedValue, pn_newitem.Tag),
               ...
               AfterChangedOutput = new Action<object>(
                  (output) =>
                  {
                     DataSet ds = output as DataSet;
                     #region Part 01
                     if (ds.Tables.Contains("GroupHeaders"))
                     {
                        lv_groupheaders.Items.Clear();
                        ds.Tables["GroupHeaders"].Rows.OfType<DataRow>()
                           .ToList()
                           .ForEach(c =>
                           {
                              lv_groupheaders.Items.Add(new ListViewItem(c["FaName"].ToString()) { Tag = c["ID"], ImageIndex = Convert.ToBoolean(c["State"]) ? 4 : 5 });
                              lv_groupheaders.Items[lv_groupheaders.Items.Count - 1].SubItems.Add(c["DbFaName"].ToString());
                           });
                     }
                     #endregion
                     #region Part 02
                     else
                     {
                        ...
                        pn_newitem.Controls.OfType<Control>()
                           .Where(c => c.Tag != null && c.Tag.ToString().StartsWith("GroupHeader_")).ToList().ForEach(c => assignValue(c, ds.Tables["GroupHeader"].Rows[0][c.Tag.ToString().Split('_')[1]]));
