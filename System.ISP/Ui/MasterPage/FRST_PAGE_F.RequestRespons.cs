...
namespace System.ISP.Ui.MasterPage
{
   partial class FRST_PAGE_F : ISendRequest
   {
      ...
      private void Set(Job job)
      {
         ...
         //Fga_Uclb_U = (iISP.FGA_UCLB_U() ?? "").Split(',').Select(c => (long?)Int64.Parse(c)).ToList();
         Lbs_CrntUser.Text = Crnt_User = iISP.GET_CRNTUSER_U(new XElement("User", new XAttribute("actntype", "001")));
         ...
         //**var Pkac = iISP.VF_AccessPackage(GetHostInfo.Output as XElement).ToList();
