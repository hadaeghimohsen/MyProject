...
{
   public partial class ACT_TRAT_F : UserControl
   {
      public ACT_TRAT_F()
      {
         InitializeComponent();
      ...
      private void RqstBnARqt1_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            //            new XElement("TrackAttendances",
            //               TratBs1.List.OfType<Data.Track_Attendance>()
            //               .Select(l =>
            //                  new XElement("TrackAttendance", 
            //                     new XAttribute("code", l.CODE),
            //                     new XAttribute("rwno", l.RWNO ?? 0),
            //                     new XAttribute("subjdesc", l.SUBJ_DESC ?? ""),
            //                     new XAttribute("attndate", l.ATTN_DATE.Value.ToString("yyyy-MM-dd hh:mm:ss")),
            //                     new XAttribute("attntype", l.ATTN_TYPE ?? ""),
            //                     new XAttribute("attnaddr", l.ATTN_ADDR ?? ""),
            //                     new XAttribute("attnrslt", l.ATTN_RSLT ?? "")
            //                  )
            //               )
            //            )
            //         )
      ...
      private void LogcBs1_CurrentChanged(object sender, EventArgs e)
      {
         try
         {
            var apon = TratBs1.Current as Data.Appointment;
      ...
      private void NewCall_Butn_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            TratBs1.AddNew();
            var trat = TratBs1.Current as Data.Track_Attendance;

            groupBox6.Visible = true;

            trat.ATTN_DATE = DateTime.Now;
            SubjDesc_Text.Focus();
         }
         catch (Exception exc)
         {
      ...
      private void RqstBnRegl01_Click(object sender, EventArgs e)
      {
         ...
         Job _InteractWithCRM =
            ...
            {                  
               ...
               new Job(SendType.SelfToUserInterface, "RQRQ_DFIN_F", 10 /* Execute Rqrq_Dfin_F */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "007")))}
            });
