...
{
   public partial class SHOW_CTRQ_F : UserControl
   {
      public SHOW_CTRQ_F()
      {
         InitializeComponent();
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         try
         {
            ...
            var test = iScsc.Tests.FirstOrDefault(t => t.RQRO_RQST_RQID == Rqid && t.FIGH_FILE_NO == FileNo && t.RECT_CODE == "004");
            if(test == null)
            {
               Gb_Test.Visible = false;
            }
            else
            {
               TestBs1.DataSource = test;
