...
{
   public partial class RLAT_CINF_F : UserControl
   {
      public RLAT_CINF_F()
      {
         InitializeComponent();         
      ...
      private async void Execute_Query()
      {
         var result = await Task.Run(() =>
         {
            using (var iCRM = new Data.iCRMDataContext(ConnectionString))
            {
               ...
               var rlatList = iCRM.Relation_Infos.Where(sc => sc.COMP_CODE == compcode).ToList();
      ...
      {         
         var comp = CompBs.List.OfType<Data.Company>().First(c => c.CODE == compcode) as Data.Company;
                  ...
                  new XElement("GMapNets",
                     ...
                     new XElement("RelationMarks",
                        new XElement("Source",
                           ...
                           new XAttribute("postadrs", comp.POST_ADRS),
                           new XAttribute("billcordx", comp.BILL_ADDR_X ?? 0),
                           new XAttribute("billcordy", comp.BILL_ADDR_Y ?? 0),
                           new XAttribute("shipcordx", comp.SHIP_ADDR_X ?? 0),
                           new XAttribute("shipcordy", comp.SHIP_ADDR_Y ?? 0)
                        ),
