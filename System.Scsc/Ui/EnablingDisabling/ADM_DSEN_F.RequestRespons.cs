...
{
   partial class ADM_DSEN_F : ISendRequest
   {
      ...
      private void LoadData(Job job)
      {
         ...
         DActgBs1.DataSource = iScsc.D_ACTGs.Where(d => Convert.ToInt32(d.VALU) >= 101);
         FighsBs1.DataSource = iScsc.Fighters.Where(f => f.CONF_STAT == "002" && (Fga_Uclb_U.Contains(f.CLUB_CODE_DNRM) || (f.CLUB_CODE_DNRM == null ? f.Club_Methods.Where(cb => Fga_Uclb_U.Contains(cb.CLUB_CODE)).Any() : false)) && Convert.ToInt32(f.ACTV_TAG_DNRM ?? "101") <= 100).OrderBy(f => f.FGPB_TYPE_DNRM);
         #endregion
