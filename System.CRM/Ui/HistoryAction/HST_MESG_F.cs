...
{
   public partial class HST_MESG_F : UserControl
   {
      public HST_MESG_F()
      {
         InitializeComponent();
            ...
            var result = await Task.Run(() =>
            {
               ...
               switch (srchtype)
               {
                  case "001":
                     return new
                     {
                        Items = db.Messages.Where(l => l.SERV_FILE_NO == fileno).ToList()
                     };
                  case "002":
                     return new
                     {
                        Items = db.Messages.Where(l => l.SERV_FILE_NO == fileno && l.MESG_DATE.Value.Date >= DateTime.Now.AddMonths(-1)).ToList()
                     };
                  case "003":
                     return new
                     {
                        Items = db.Messages.Where(l => l.SERV_FILE_NO == fileno && l.MESG_DATE.Value.Date >= DateTime.Now.AddDays(-7)).ToList()
                     };
                  case "004":
                     return new
                     {
                        Items = db.Messages.Where(l => l.SERV_FILE_NO == fileno && l.MESG_DATE.Value.Date == DateTime.Now.AddDays(-1).Date).ToList()
                     };
                  case "005":
                     return new
                     {
                        Items = db.Messages.Where(l => l.SERV_FILE_NO == fileno && l.MESG_DATE.Value.Date == DateTime.Now.Date).ToList()
                     };
                  case "006":
                     return new
                     {
                        Items = db.Messages.Where(l => l.SERV_FILE_NO == fileno && l.MESG_DATE.Value.Date == rqstToDate.Value.Date).ToList()
                     };
                  default:
                     return new { Items = (List<Data.Message>)null };
               }
            });

            iCRM = new Data.iCRMDataContext(ConnectionString);
            MesgBs.DataSource = result.Items;
         }
         catch (Exception exc)
         {
      ...
      private void Mesg_Gv_DoubleClick(object sender, EventArgs e)
      {
         try
         {
            var crnt = MesgBs.Current as Data.Message;
