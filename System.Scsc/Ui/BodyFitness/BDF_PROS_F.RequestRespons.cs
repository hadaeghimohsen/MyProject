...
{
   partial class BDF_PROS_F : ISendRequest
   {
      ...
      {
         FighBs1.DataSource = iScsc.Fighters.Where(f => f.CONF_STAT == "002" && f.FGPB_TYPE_DNRM != "002" && f.FGPB_TYPE_DNRM != "003" && f.FGPB_TYPE_DNRM != "007" && !f.NAME_DNRM.Contains("مشتری, جلسه ای") && (Fga_Uclb_U.Contains(f.CLUB_CODE_DNRM) || (f.CLUB_CODE_DNRM == null ? f.Club_Methods.Where(cb => Fga_Uclb_U.Contains(cb.CLUB_CODE)).Any() : false)) && Convert.ToInt32(f.ACTV_TAG_DNRM ?? "101") >= 101).OrderBy(f => f.FGPB_TYPE_DNRM);
         DBodyBs1.DataSource = iScsc.D_BODies;
         DBmirBs1.DataSource = iScsc.D_BMIRs;
         DActvBs1.DataSource = iScsc.D_ACTVs;
         DIdtpBs1.DataSource = iScsc.D_IDTPs;
         DYsnoBs1.DataSource = iScsc.D_YSNOs;
         DMstpBs1.DataSource = iScsc.D_MSTPs;
         DCyclBs1.DataSource = iScsc.D_CYCLs;
         DMsrtBs1.DataSource = iScsc.D_MSRTs;
