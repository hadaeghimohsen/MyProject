   ...
   partial class SERV_DART_F : ISendRequest
   {
      ...
      private void LoadData(Job job)
      {
         try
         {
            ...
            BIL_DEBITBindingSource.DataSource = access_entity.Run_Qury_U("SELECT * FROM Bil_Debit WHERE Serv_File_No = :p0", new OracleParameter[] { new OracleParameter(":p0", FileNo) });
             BIL_ACCOUNTBindingSource.DataSource = access_entity.Run_Qury_U("SELECT Serv_File_No, Rwno,Bill_No_Dnrm,Regn_Code,Actn_Code,DECODE (Debt_Stat, 1, Amnt, NULL) AS Bed_Amnt,DECODE (Debt_Stat, 2, Amnt, NULL) AS Bes_Amnt,Debt_Stat,Remn_Dnrm, Lett_No_Dnrm,Lett_Date_Dnrm, Amnt_Date FROM Bil_Account WHERE Serv_File_No = :p0 ORDER BY Rwno DESC", new OracleParameter[] { new OracleParameter(":p0", FileNo) });
             BIL_RCPT_ROW_ANNOUNCEMENTBindingSource.DataSource = access_entity.Run_Qury_U("SELECT B5.Code AS RCPA_BNKB_BANK_CODE,B5.Bank_Desc,B4.Code AS RCPA_BNKB_CODE,B4.NAME AS Brnc_Desc,B3.File_Name AS RCPN_FILE_NAME,B2.Rpan_Updt_Date AS RCPA_RPAN_UPDT_DATE ,B1.Rcpa_Rcpn_Rcpt_Seq,B1.Rcpa_Annc_Date,B1.Bill_Bill_No,B1.Rwno,B1.Rcpt_Amnt,B1.Insm_Ser_Dnrm FROM Bil_Rcpt_Row_Announcement B1,Bil_Rcpt_Announcement B2,Bil_Rcpt_Name B3,Bank_Branch B4,Bank B5 WHERE B1.Rcpa_Rcpn_Rcpt_Seq = B2.Rcpn_Rcpt_Seq AND B1.Rcpa_Annc_Date = B2.Annc_Date AND B1.Rcpa_Bnkb_Bank_Code = B2.Bnkb_Bank_Code AND B1.Rcpa_Bnkb_Code = B2.Bnkb_Code AND B2.Rcpn_Rcpt_Seq = B3.Rcpt_Seq AND B2.Bnkb_Bank_Code = B4.Bank_Code AND B2.Bnkb_Code = B4.Code AND B4.Bank_Code = B5.Code AND B1.Bill_Serv_File_No = :p0 AND B1.Actn_Stat = '1' ORDER BY B1.Rcpa_Rcpn_Rcpt_Seq DESC", new OracleParameter[] { new OracleParameter(":p0", FileNo) });
