...
{
   partial class SettingsSystemTinyLock : ISendRequest
   {
      ...
      {
         SubSysBs.DataSource = iProject.Sub_Systems.Where(s => s.SUB_SYS == 5 || s.SUB_SYS == 11 || s.SUB_SYS == 12);
         job.Status = StatusType.Successful;
      }
      ...
      {
         SwitchButtonsTabPage(AppsPackages_Butn);
         job.Status = StatusType.Successful;
      }
   }
}
