...
{
   public partial class UCC_SAVE_F : UserControl
   {
      public UCC_SAVE_F()
      {
         InitializeComponent();
      ...
      private void Bt_DontPrntCard_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            CrntRqid.Request_Rows.ToList()
               .ForEach(rr =>
               {
                  iScsc.UPD_MBSP_P(rr.RQST_RQID, rr.RWNO, rr.FIGH_FILE_NO, "001", "001", rr.Member_Ships.Single(ms => ms.RECT_CODE == "001").STRT_DATE.Value.Date, rr.Member_Ships.Single(ms => ms.RECT_CODE == "001").END_DATE.Value.Date, 1);
               });
            requeryData = true;
         }
         catch (Exception ex)
