...
{
   public partial class ADM_HRSC_F : UserControl
   {
      public ADM_HRSC_F()
      {
         InitializeComponent();
         ...
         try
         {
            if (tb_master.SelectedTab == tp_001)
            {
               ...
               var result = await Task.Run(() =>
               {
                  ...
                  {
                     var Rqids = context.VF_Requests(new XElement("Request"))
                        .Where(rqst =>
                              rqst.RQTP_CODE == "023" &&
                              rqst.RQST_STAT == "001" &&
                              rqst.RQTT_CODE == "004" &&
      ...
      private void Btn_RqstRqt1_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            if (Rqst == null || Rqst.RQID >= 0)
            {
               ...
               if (FileNo_Lookup.EditValue.ToString() == "") return;
                  ...
                  iScsc.HRS_CRQT_P(
                        new XElement("Process",
                           new XElement("Request",
                              ...
                              new XAttribute("rqtpcode", "023"),
                              new XAttribute("rqttcode", "004"),
                              new XAttribute("prvncode", "017"),
                              new XAttribute("regncode", "001"),
                              new XElement("Fighter",
                                 new XAttribute("fileno", Rqst == null ? FileNo_Lookup.EditValue : Rqst.Fighters.FirstOrDefault() == null ? FileNo_Lookup.EditValue : Rqst.Fighters.FirstOrDefault().FILE_NO),
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "023")))}
                  })
               );
         }         
      }
