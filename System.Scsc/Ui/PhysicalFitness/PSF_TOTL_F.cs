...
{
   public partial class PSF_TOTL_F : UserControl
   {
      public PSF_TOTL_F()
      {
         InitializeComponent();
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         ...
         OldRecdBs1.DataSource = iScsc.Physical_Fitnesses.Where(ex => ex.FIGH_FILE_NO == Rqro.FIGH_FILE_NO && ex.RECT_CODE == "004" && ex.Fighter.PSFN_RWNO_DNRM == ex.RWNO);
      }
                     ...
                     new XElement("Request",
                        ...
                        new XElement("Request_Row",
                           ...
                           new XElement("Physical_Fitness",
                              new XAttribute("pullup", PULL_UPTextEdit1.Text),
                              new XAttribute("pushup", PUSH_UPTextEdit1.Text),
                              new XAttribute("squtjump", SQUT_JUMPTextEdit1.Text),
                              new XAttribute("squttrst", SQUT_TRSTTextEdit1.Text),
                              new XAttribute("situp", SIT_UPTextEdit1.Text)
                           )
                        )
                     )
                  )
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "005")))}
                  })
               );
         } 
      }
