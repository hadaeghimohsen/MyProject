...
{
   public partial class CMP_RQST_F : UserControl
   {
      public CMP_RQST_F()
      {
         InitializeComponent();
      ...
      private void Tsb_SaveItem_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            Func<long?, XElement> InsertCampData
               = new Func<long?, XElement>(
                  (long? fileno) =>
                  {
                     var campData = (campititionsBindingSource.Current as Data.Campitition);
                     if (campData != null)
                     {
                        return new XElement("Campitition",
                              new XAttribute("campdate", campData.CAMP_DATE.Value.ToString("yyyy-MM-dd")),
                              new XAttribute("placadrs", campData.PLAC_ADRS),
                              new XAttribute("sectnumb", campData.SECT_NUMB)
                           );
                     }
                     return null;
                  }
