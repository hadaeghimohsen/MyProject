...
{
   public partial class MCC_TOTL_F : UserControl
   {
      public MCC_TOTL_F()
      {
         InitializeComponent();
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         ...
         OldRecdBs1.DataSource = iScsc.Member_Ships.Where(c => c.FIGH_FILE_NO == Rqro.FIGH_FILE_NO && c.RECT_CODE == "004" && c.Fighter.MBSP_RWNO_DNRM == c.RWNO);
      }
            ...
            iScsc.MCC_RQST_P(
               new XElement("Process",
                  new XElement("Request",
                     ...
                     new XAttribute("rqtpcode", "010"),
                     new XAttribute("rqttcode", Rqst == null ? RQTT_CODE_LookUpEdit.EditValue ?? "" : Rqst.RQTT_CODE),
                     new XElement("Request_Row",
                        new XAttribute("fileno", Figh == null ? FILE_NO_LookUpEdit.EditValue ?? "" : Figh.FILE_NO),
                        new XElement("Member_Ship",
                           new XAttribute("strtdate", STRT_DATE_DateTime.Value == null ? "" : STRT_DATE_DateTime.Value.Value.ToString("yyyy-MM-dd")),
                           new XAttribute("enddate", END_DATE_DateTime.Value == null ? "" : END_DATE_DateTime.Value.Value.ToString("yyyy-MM-dd"))
                        )
                     )
                  )
               )
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "010")))}
                  })
               );
         }
      }
