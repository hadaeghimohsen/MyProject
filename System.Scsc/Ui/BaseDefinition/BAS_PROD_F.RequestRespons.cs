...
{
   partial class BAS_PROD_F : ISendRequest
   {
      ...
      private new void Paint(Job job)
      {
         ...
         Job _Paint = new Job(SendType.External, "Desktop",
            new List<Job>
            {
               ...
               new Job(SendType.SelfToUserInterface, "Wall", 15 /* Execute Push */) {  Input = new List<object> { string.Format("Scsc:{0}", this.GetType().Name), this }  },
               new Job(SendType.SelfToUserInterface, "Wall", 00 /* Execute PastManualOnWall */) {  Input = new List<object> { this, "right:in-screen:stretch:center" } }               
            });
         _DefaultGateway.Gateway(_Paint);
      ...
      {
         DSaleBs.DataSource = iScsc.D_SALEs;
         DStokBs.DataSource = iScsc.D_STOKs;
         job.Status = StatusType.Successful;
      }
      ...
      private void Actn_CalF_P(Job job)
      {
         ...
         if(xinput != null)
         {
            switch (xinput.Attribute("formstat").Value)
            {
               case "show":
                  epitcode = Convert.ToInt64(xinput.Attribute("epitcode").Value);
                  SaleProd_Butn.Visible = SaleProd_Pn.Visible = false;
                  pydtcode = null;
                  break;
               case "sale":                  
                  pydtcode = Convert.ToInt64(xinput.Attribute("pydtcode").Value);
                  epitcode = Convert.ToInt64(xinput.Attribute("epitcode").Value);                  
                  SaleProd_Butn.Visible = SaleProd_Pn.Visible = true;
                  break;
               default:
                  break;
            }
