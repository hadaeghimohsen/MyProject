...
namespace System.Scsc.Ui.Admission
{
   partial class ADM_FIGH_F : ISendRequest
   {
      ...
      finishcommand:
         ...
         CbmtBs1.DataSource = iScsc.Club_Methods.Where(cbmt => Fga_Uclb_U.Contains(cbmt.CLUB_CODE) && cbmt.MTOD_STAT == "002" && Convert.ToInt32(cbmt.Fighter.ACTV_TAG_DNRM ?? "101") >= 101)/*.OrderBy(cm => new { cm.CLUB_CODE, cm.COCH_FILE_NO, cm.DAY_TYPE, cm.STRT_TIME })*/;
         job.Status = StatusType.Successful;
      }
