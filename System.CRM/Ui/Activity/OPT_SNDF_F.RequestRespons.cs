...
{
   partial class OPT_SNDF_F : ISendRequest
   {
      ...
      private void Actn_CalF_P(Job job)
      {
         ...
         if(xinput != null)
         {
            ...
            if (xinput.Attribute("sendtype") != null)
               sendtype = xinput.Attribute("sendtype").Value;
            else
               sendtype = "";
            ...
            SndfBs.List.Clear();
            if (xinput.Attribute("sendfiletype").Value == "new")
            {
               SndfBs.AddNew();
               ...
               {
                  SendType_Lov.Enabled = true;
               }

               SndfBs.EndEdit();
            }
            else if (xinput.Attribute("sendfiletype").Value == "edit")
            {
               SndfBs.DataSource = iCRM.Send_Files.FirstOrDefault(sd => sd.SERV_FILE_NO == fileno && sd.SFID == Convert.ToInt64(xinput.Attribute("sfid").Value));               
            }
         }
