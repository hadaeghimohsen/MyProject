...
{
   public partial class SettingsBlockGateway : UserControl
   {
      public SettingsBlockGateway()
      {
         InitializeComponent();
      }
      ...
      {
         var gatewayTag = UserGateway_Gv.Tag as Data.Gateway;
         ...
         var result = await Task.Run(() =>
         {
            using (var ctx = new Data.iProjectDataContext(ConnectionString))
            {
               return new
               {
                  Gateways = ctx.Gateways.Where(g => g.AUTH_TYPE_DNRM == "003").ToList(),
                  UserGateways = gatewayTag != null ? ctx.User_Gateways.Where(ug => ug.Gateway == gatewayTag && ug.RWNO == ctx.User_Gateways.Where(ugt => ugt.Gateway == gatewayTag && ugt.USER_ID == ug.USER_ID).Max(ugt => ugt.RWNO)).ToList() : null,
                  GatewayInfo = gatewayTag != null ? string.Format("{0}<br><color=DimGray><size=10>{1}</size></color><br>", gatewayTag.COMP_NAME_DNRM, gatewayTag.IP_DNRM) : ""
               };
            }
         });
         ...
         CreateGatewayMenu();
         UserGatewayBs.Clear();
         GatewayInfo_Lbl.Text = result.GatewayInfo;
         if (result.UserGateways != null)
         {
      ...
      private void UnAuthorise_Butn_Click(object sender, EventArgs e)
      {
         try
         {
            var usgw = UserGatewayBs.Current as Data.User_Gateway;
            if (usgw == null || MessageBox.Show(this, "آیا با غیرمجاز کردن سیستم موافق هستید؟", "غیرمجاز شدن سیستم", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) != DialogResult.Yes) return;
