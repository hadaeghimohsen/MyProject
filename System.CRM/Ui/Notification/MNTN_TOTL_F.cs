...
{
   public partial class MNTN_TOTL_F : UserControl
   {
      public MNTN_TOTL_F()
      {
         InitializeComponent();
      ...
      private void Notification_Gv_CustomUnboundColumnData(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
      {
         ...
         {
            var alrmdate = ((DateTime)view.GetListSourceRowCellValue(e.ListSourceRowIndex, "MNTN_DATE"));
      ...
      private void Lnk_MarkAllAsRead_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
      {
         try
         {
            ...
            iCRM.Mentions.Where(m => m.Job_Personnel.USER_NAME.ToLower() == CurrentUser.ToLower() && m.MNTN_DATE.Value.Date <= DateTime.Now.Date && m.READ_MNTN == "001").ToList().ForEach(r => r.READ_MNTN = "002");
            iCRM.SubmitChanges();
            requery = true;
         }
         catch (Exception exc)
      ...
      private void Notf_Butn_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
      {
         try
         {
            var crnt = MntnBs.Current as Data.Mention;
      ...
      private void Mentioned_Gv_DoubleClick(object sender, EventArgs e)
      {
