...
{
   partial class REGL_DFIN_F : ISendRequest
   {
               ...
               new List<Job>
               {
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        {
                           Input = new List<string> {"<Privilege>27</Privilege><Sub_Sys>10</Sub_Sys>", "DataGuard"},
                           AfterChangedOutput = new Action<object>((output) => {
                              if ((bool)output)
                                 return;
                              #region Show Error
                              job.Status = StatusType.Failed;
                              MessageBox.Show(this, "خطا - عدم دسترسی به ردیف 27 امنیتی", "خطا دسترسی");
                              #endregion                           
                           })
                        },
                        #endregion                        
      ...
      private void LoadData(Job job)
      {
         ...
         {            
            ...
            DrgtpBs.DataSource = iISP.D_RGTPs;
         }
         Execute_Query();
         job.Status = StatusType.Successful;
      }
