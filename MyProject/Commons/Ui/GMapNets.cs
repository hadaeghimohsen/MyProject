...
using MyProject.Commons.Ui.GMapNet.BSE.Windows.Forms.Renderer;
using GMap.NET.WindowsForms.Markers;
using System.Threading;
using System.Drawing.Drawing2D;
using System.Net;
using System.IO;
using MyProject.Commons.Ui.GMapNet.Others;
...
{
   public partial class GMapNets : UserControl
   {
      ...
      public GMapNets()
      {
         ...
         if (!GMapControl.IsDesignerHosted)
         {
            ...
            MainMap.Position = new PointLatLng(29.622192, 52.522329);
            MainMap.MinZoom = 0;
            MainMap.MaxZoom = 24;
            MainMap.Zoom = 9;
            ...
            //if(false)
            {
               ...
               {
                  PointLatLng? pos = GMapProviders.GoogleMap.GetPoint(/*"Lithuania, Vilnius"*/ textBoxGeo.Text, out status);
                  if (pos != null && status == GeoCoderStatusCode.G_GEO_SUCCESS)
                  {
                     ...
                     myCity.ToolTipText = "Welcome to Iran! ;}";
                     objects.Markers.Add(myCity);
                  }
               }

               // add some points in lithuania
               AddLocation("Fars, Shiraz");
               //AddLocation("Klaipėda");
               //AddLocation("Šiauliai");
               //AddLocation("Panevėžys");
      ...
      private void Back_Butn_Clicked(object sender, EventArgs e)
      {
      ...
      void transport_DoWork(object sender, DoWorkEventArgs e)
      {
         while(!transportWorker.CancellationPending)
         {
            try
            {
               ...
               lock(trolleybus)
               {
                  Stuff.GetVilniusTransportData(MyProject.Commons.Ui.Code.TransportType.TrolleyBus, string.Empty, trolleybus);
               }

               lock(bus)
               {
                  Stuff.GetVilniusTransportData(MyProject.Commons.Ui.Code.TransportType.Bus, string.Empty, bus);
               }
               #endregion
      ...
      List<IpInfo> GetIpHostInfo(string iplist)
      {
         ...
         {
            IpInfo val = null; // IpCache.GetDataFromCache(ip);
            if(val != null)
            {
               ret.Add(val);
            }
      ...
      void AddLocation(string place)
      {
         GeoCoderStatusCode status = GeoCoderStatusCode.Unknow;
         PointLatLng? pos = GMapProviders.GoogleMap.GetPoint("Iran, " + place, out status);
         if(pos != null && status == GeoCoderStatusCode.G_GEO_SUCCESS)
         {
         ...
         if(!UserAcceptedLicenseOnce)
         {
            if(File.Exists(AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + "License.txt"))
            {
               string txt = File.ReadAllText(AppDomain.CurrentDomain.BaseDirectory + Path.DirectorySeparatorChar + "License.txt");
                {
                    var d = new MyProject.Commons.Ui.GMapNet.Others.Message();
      ...
      private void button8_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            {
               var result = MainMap.SetPositionByKeywords(textBoxGeo.Text);
               switch (result)
               {
                  ...
                  case GeoCoderStatusCode.G_GEO_SUCCESS:
                     textBoxLat.Text = textBoxLatCurrent.Text;
                     textBoxLng.Text = textBoxLngCurrent.Text;
                     break;
            ...
            new Job(SendType.External, "localhost", xinput.Attribute("formcaller").Value /* Goto Route */, Convert.ToInt32(xinput.Attribute("callback").Value) /* Execute Callback Method */, SendType.SelfToUserInterface)
            {
               Input = 
                  new XElement("GMapNets",
                     new XAttribute("outputtype", xinput.Attribute("outputtype").Value),
                     new XAttribute("cordx", textBoxLatCurrent.Text),
                     new XAttribute("cordy", textBoxLngCurrent.Text),
                     new XAttribute("zoom", trackBar1.Value)
                  )
            }
         );
      }
      ...
      {
         MessageBox.Show(string.Format("{0} :> {1} ({2}, {3})", item.Tag, item.ToolTipText, item.LocalPosition.X, item.LocalPosition.Y));
      }

      private void textBoxGeo_TextChanged(object sender, EventArgs e)
      {
         KeywordFind_Rb.Checked = true;
      }
