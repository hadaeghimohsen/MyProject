...
namespace System.ServiceDefinition.GrpHdr.Ui
{
   public partial class Create : UserControl
   {
      ...
      private void sb_creategroupheader_Click(object sender, EventArgs e)
      {
         ...
         Func<string> XmlData = new Func<string>(
            () =>
            {
               string groupname = string.Format("<FaName>{0}</FaName><EnName>{1}</EnName>", te_titlefa.Text, te_titleen.Text);
               string roles = "";
               ccbe_roles.Properties.GetItems().OfType<CheckedListBoxItem>().Where(r => r.CheckState == CheckState.Checked).ToList().ForEach(role => roles += string.Format("<RoleID>{0}</RoleID>", role.Value));

               return string.Format("<Create>{0}<Roles>{1}</Roles></Create>",groupname, roles);
            });
                  ...
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        new Job(SendType.Self, 07 /* Execute DoWork4AccessPrivilege */)
                        {
                           ...
                           {
                              "<Privilege>3</Privilege><Sub_Sys>1</Sub_Sys>",
                              "DataGuard"
                           },
                        ...
                        new Job(SendType.Self, 04 /* Execute DoWork4Odbc */){
                           Input = new List<object>
                           {
                              ...
                              "xml",
                              XmlData(),
                              "{ Call ServiceDef.CreateNewGroupHeader(?) }",
                              "iProject",
                              "scott"
                           }
                        },
