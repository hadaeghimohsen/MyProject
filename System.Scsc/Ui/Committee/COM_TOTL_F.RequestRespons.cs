...
{
   partial class COM_TOTL_F : ISendRequest
   {
      ...
      private void ProcessCmdKey(Job job)
      {
         ...
         {
            if (!(Btn_RqstBnARqt1.Focused || Btn_RqstBnASav1.Focused || Btn_RqstBnDelete1.Focused || Btn_NewRecord.Focused))
               SendKeys.Send("{TAB}");
      ...
      {
         #region Rqsw block
         FighsBs1.DataSource = iScsc.Fighters.Where(f => f.CONF_STAT == "002" /*&& (f.FGPB_TYPE_DNRM == "001" || f.FGPB_TYPE_DNRM == "005" || f.FGPB_TYPE_DNRM == "006")*/ && Fga_Urgn_U.Split(',').Contains(f.REGN_PRVN_CODE + f.REGN_CODE) && Fga_Uclb_U.Contains(f.CLUB_CODE_DNRM));
         DDsctBs1.DataSource = iScsc.D_DSCTs;
         DSxtpBs1.DataSource = iScsc.D_SXTPs;
         DMttpBs1.DataSource = iScsc.D_MTTPs;
         DUsrtBs1.DataSource = iScsc.D_USRTs;
