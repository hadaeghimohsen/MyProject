...
{
   partial class SettingsNetworkConnection : ISendRequest
   {
      ...
      private void Set(Job job)
      {
         ...
         HostInfo = GetHostInfo.Output as XElement;
               ...
               new List<Job>
               {
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        {
                           Input = new List<string> {"<Privilege>32</Privilege><Sub_Sys>0</Sub_Sys>", "DataGuard"},
                           AfterChangedOutput = new Action<object>((output) => {
                              if ((bool)output)
                                 return;
                              #region Show Error
                              job.Status = StatusType.Failed;
                              MessageBox.Show(this, "خطا - عدم دسترسی به ردیف 32 امنیتی", "خطا دسترسی");
                              #endregion                           
                           })
                        },
                        #endregion                        
      ...
      private void ActionCallWindow(Job job)
      {
         if(job.Input != null)
         {
            ...
            {
               case "tp_001":
                  SwitchButtonsTabPage(ConnectionStatus_Butn);
                  break;
               case "tp_002":
                  SwitchButtonsTabPage(ConnectionList_Butn);
                  break;
               default:
                  SwitchButtonsTabPage(ConnectionStatus_Butn);
                  break;
