...
{
   public partial class EXM_TOTL_F : UserControl
   {
      public EXM_TOTL_F()
      {
         InitializeComponent();
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         ...
         OldRecdBs1.DataSource = iScsc.Exams.Where(ex => ex.FIGH_FILE_NO == Rqro.FIGH_FILE_NO && ex.RECT_CODE == "004" && ex.Fighter.EXAM_RWNO_DNRM == ex.RWNO);
      }
                  ...
                  new XElement("Process",
                     new XElement("Request",
                        ...
                        new XElement("Request_Row",
                           ...
                           new XElement("Exam",
                              new XAttribute("type", TYPE_LookUpEdit1.EditValue),
                              new XAttribute("time", TIME_TextEdit1.Text),
                              new XAttribute("cachnumb", CACH_NUMB_TextEdit1.Text),
                              new XAttribute("stephegh", STEP_HEGH_TextEdit1.Text),
                              new XAttribute("wegh", WEGH_TextEdit1.Text)
                           )
                        )
                     )
                  )
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "007")))}
                  })
               );
         }
      }
