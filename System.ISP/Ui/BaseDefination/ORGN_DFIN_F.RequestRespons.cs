...
{
   partial class ORGN_DFIN_F : ISendRequest
   {
               ...
               new List<Job>
               {
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        {
                           Input = new List<string> {"<Privilege>51</Privilege><Sub_Sys>10</Sub_Sys>", "DataGuard"},
                           AfterChangedOutput = new Action<object>((output) => {
                              if ((bool)output)
                                 return;
                              #region Show Error
                              job.Status = StatusType.Failed;
                              MessageBox.Show(this, "خطا - عدم دسترسی به ردیف 51 امنیتی", "خطا دسترسی");
                              #endregion                           
                           })
                        },
                        #endregion                        
      ...
      private void LoadData(Job job)
      {
         if(!firstloaded)
         {
            ...
            DcetpBs.DataSource = iISP.D_CETPs;
            DdsatBs.DataSource = iISP.D_DSATs;
         }
