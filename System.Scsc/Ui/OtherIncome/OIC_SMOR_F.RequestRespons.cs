...
{
   partial class OIC_SMOR_F : ISendRequest
   {
      ...
      private bool isFristLoaded = false;
      ...
      {
         if (isFristLoaded) goto finishcommand;
      ...
      private void Actn_CalF_P(Job job)
      {
         ...
         {
            if ((job.Input as XElement).Attribute("type").Value == "tp_001")
            {
               INSR_NUMB_TextEdit.Text = CARD_NUMBTextEdit.Text = FNGR_PRNT_TextEdit.Text = (job.Input as XElement).Attribute("enrollnumber").Value;
               INSR_DATE_PersianDateEdit.Value = DateTime.Now;
            }
            else if ((job.Input as XElement).Attribute("type").Value == "tp_002")
               Txt_CardNumb002.Text = FNGR_PRNT_TextEdit02.Text = (job.Input as XElement).Attribute("enrollnumber").Value;
         }
         else
            Execute_Query();
