...
{
   partial class SettingsPersonalization : ISendRequest
   {
               ...
               new List<Job>
               {
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        {
                           Input = new List<string> {"<Privilege>33</Privilege><Sub_Sys>0</Sub_Sys>", "DataGuard"},
            ...
            if (InvokeRequired)
            {
               Invoke(
                  new Action(
                     () =>
                     {
                        var bmps = Directory.GetFiles(@".\Media\SubSys\Kernel\Desktop\Wallpaper\Anar").Take(5);
                        Flp_BackgroundSelection.Controls.Clear();
                        bmps.ToList()
                           .ForEach(
                              img =>
                              {
                                 var p = new Panel() { BackgroundImage = Image.FromFile(img), BackgroundImageLayout = ImageLayout.Stretch, Size = new Size(97, 94), Margin = new Padding(3, 3, 1, 1), Cursor = Cursors.Hand, Tag = img };
                                 p.Click += BackgroundSelection_Click;
                                 Flp_BackgroundSelection.Controls.Add(
                                    p
                                 );
                              }
                           );
      ...
      private void ActionCallWindow(Job job)
      {
         ...
         if(job.Input != null)
         {
            switch (job.Input.ToString())
            {
               case "simplecolor":
                  SwitchButtonsTabPage(SimpleColor_Butn);
                  break;
               case "lockscreen":
                  SwitchButtonsTabPage(LockScreen_Butn);
                  break;
               case "icons":
                  SwitchButtonsTabPage(Icons_Butn);
                  break;
               case "start":
                  SwitchButtonsTabPage(Start_Butn);
                  break;
               case "taskbar":
                  SwitchButtonsTabPage(Taskbar_Butn);
                  break;
            }
         }
         else
            SwitchButtonsTabPage(BackGround_Butn);
         job.Status = StatusType.Successful;
      }
   }
}
