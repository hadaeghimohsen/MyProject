   ...
   public partial class INF_CNTR_F : UserControl
   {
      ...
      private async void Execute_Query()
      {
         try
         {
            ...
            var _queryAllRequest = queryAllRequest;
            var _rqid = rqid;
            ...
            var result = await Task.Run(() =>
            {
               ...
               {
                  if (_formType == "normal") // in process
                  {
                     var Rqids = db.VF_Requests(new XElement("Request", new XAttribute("cretby", _queryAllRequest ? "" : _currentUser)))
                        .Where(rqst =>
                              rqst.RQTP_CODE == "016" &&
                              rqst.RQST_STAT == "001" &&
                              rqst.RQTT_CODE == "004" &&
                  ...
                  else if(_formType == "showhistory") // ended
                  {
                     var Rqids = db.VF_Requests(new XElement("Request", new XAttribute("cretby", _queryAllRequest ? "" : _currentUser)))
                        .Where(rqst =>
                              rqst.RQTP_CODE == "015" &&
                  ...
                  return new { requests = new List<Data.Request>() };
               }
            });

            iCRM = new Data.iCRMDataContext(ConnectionString);
            RqstBs.DataSource = result.requests;
         ...
         {            
            ...
            iCRM.OPR_CNTR_P(
               new XElement("Request",
                  ...
                  new XAttribute("cntrtype", cntrtype),                  
                  new XElement("Request_Row",
                     ...
                     new XElement("Contract",
                        ...
                        new XAttribute("name", Name_Txt.Text),
                        ...
                        new XAttribute("tempapbscode", TempApbsCode_Lov.EditValue),
                        new XAttribute("dscntype", DscnType_Lov.EditValue),
                        new XAttribute("servlevl", ServLevl_Lov.EditValue),
                        new XAttribute("cmnt", Cmnt_Txt.EditValue),
                        new XAttribute("billstrtdate", BillStrtDate_Dt.Value.HasValue ? BillStrtDate_Dt.Value.Value.ToString("yyyy-MM-dd") : ""),
                        new XAttribute("billenddate", BillEndDate_Dt.Value.HasValue ? BillEndDate_Dt.Value.Value.ToString("yyyy-MM-dd") : ""),
                        new XAttribute("billcncldate", BillCnclDate_Dt.Value.HasValue ? BillCnclDate_Dt.Value.Value.ToString("yyyy-MM-dd") : ""),
                        new XAttribute("billferq", BillFerq_Lov.EditValue),
                        new XAttribute("trcbtcid", TrcbTcid_Lov.EditValue),
                        new XAttribute("totlpric", TotlPric_Txt.EditValue),
                        new XAttribute("totldscn", TotlDscn_Txt.EditValue),
      ...
      private void AddNote_Butn_Click(object sender, EventArgs e)
      {
         ...
         if (cntr == null || cntr.Request_Row == null) return;

         if (NoteBs.List.OfType<Data.Note>().Any(n => n.NTID == 0)) return;

         NoteBs.AddNew();
         var note = NoteBs.Current as Data.Note;
         note.Request_Row = cntr.Request_Row;
         note.SERV_FILE_NO = cntr.SRPB_SERV_FILE_NO;
         note.COMP_CODE_DNRM = cntr.COMP_CODE;
