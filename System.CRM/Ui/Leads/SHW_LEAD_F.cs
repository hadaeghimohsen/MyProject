...
namespace System.CRM.Ui.Leads
{
   public partial class SHW_LEAD_F : UserControl
   {
      ...
      private void ImageProfile_Butn_Click(object sender, EventArgs e)
      {
         try
         {
            var lead = ServBs.Current as Data.VF_ServicesResult;
            if (lead == null) return;
            ...
            Job _InteractWithCRM =
              new Job(SendType.External, "Localhost",
                 ...
                 {                  
                   ...
                   new Job(SendType.SelfToUserInterface, "INF_LEAD_F", 10 /* Execute ACTN_CALF_P */){Input = new XElement("Service", new XAttribute("fileno", lead.FILE_NO))},
                 });
            _DefaultGateway.Gateway(_InteractWithCRM);
         }
         catch { }
      ...
      private void SendMesagges_Butn_Click(object sender, EventArgs e)
      {

      }

      private void ServBs_ListChanged(object sender, ListChangedEventArgs e)
      {
         if (ServBs.List.Count != 0)
            SendEmails_Butn.Visible = SendMesagges_Butn.Visible = true;
         else
            SendEmails_Butn.Visible = SendMesagges_Butn.Visible = false;
      }

      private void ServInfo_Pikb_PickCheckedChange(object sender)
      {
