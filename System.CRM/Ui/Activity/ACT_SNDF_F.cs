...
{
   public partial class ACT_SNDF_F : UserControl
   {
      public ACT_SNDF_F()
      {
         InitializeComponent();
      ...
      private void RqstBnARqt1_Click(object sender, EventArgs e)
      {
         try
         {
            ...
            //            new XElement("SendFiles",
            //               SndfBs1.List.OfType<Data.Send_File>()
            //               .Select(l =>
            //                  new XElement("SendFile", 
            //                     new XAttribute("code", l.CODE),
            //                     new XAttribute("rwno", l.RWNO ?? 0),
            //                     new XAttribute("subjdesc", l.SUBJ_DESC ?? ""),
            //                     new XAttribute("senddate", l.SEND_DATE.Value.ToString("yyyy-MM-dd hh:mm:ss")),
            //                     new XAttribute("sendstat", l.SEND_STAT ?? ""),
            //                     new XAttribute("vlu1", l.VLU1 ?? ""),
            //                     new XAttribute("vlu2", l.VLU2 ?? ""),
            //                     new XAttribute("sendtype", l.SEND_TYPE ?? ""),
            //                     new XAttribute("filepath", l.FILE_PATH ?? "")
            //                  )
            //               )
            //            )
            //         )
      ...
      private void LogcBs1_CurrentChanged(object sender, EventArgs e)
      {
         try
         {
            var send = SndfBs1.Current as Data.Send_File;
      ...
      private void Recall_Butn_Click(object sender, EventArgs e)
      {
         try
         {
            //var currentLogCall = SndfBs1.Current as Data.Log_Call;
            //currentLogCall.LOG_RSLT += string.Format("پیگیری برای {0} دقیقه دیگر", Minute_Text.Text);
            //SndfBs1.AddNew();
            //var log = SndfBs1.Current as Data.Log_Call;
      ...
      private void Browse_Butn_Click(object sender, EventArgs e)
      {
         if (FileBrowse_Ofd.ShowDialog() != DialogResult.OK) return;

         FilePath_Text.Text = FilePath_Text.Text + FileBrowse_Ofd.FileName + "\r\n";
      }
      ...
      private void RqstBnRegl01_Click(object sender, EventArgs e)
      {
         ...
         Job _InteractWithCRM =
            ...
            {                  
               ...
               new Job(SendType.SelfToUserInterface, "RQRQ_DFIN_F", 10 /* Execute Rqrq_Dfin_F */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "006")))}
            });
