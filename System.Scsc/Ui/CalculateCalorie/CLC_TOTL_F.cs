...
{
   public partial class CLC_TOTL_F : UserControl
   {
      public CLC_TOTL_F()
      {
         InitializeComponent();
      ...
      private void RqroBs1_CurrentChanged(object sender, EventArgs e)
      {
         ...
         OldRecdBs1.DataSource = iScsc.Calculate_Calories.Where(c => c.FIGH_FILE_NO == Rqro.FIGH_FILE_NO && c.RECT_CODE == "004" && c.Fighter.CLCL_RWNO_DNRM == c.RWNO);
      }
                  ...
                  new XElement("Process",
                     new XElement("Request",
                        ...
                        new XElement("Request_Row",
                           ...
                           new XElement("Calculate_Calorie",
                              new XAttribute("hegh", Hegh_TextEdit.Text),
                              new XAttribute("wegh", Wegh_TextEdit.Text),
                              new XAttribute("trantime", Tran_Time_TextEdit.Text)
                           )
                        )
                     )
                  )
            ...
            _DefaultGateway.Gateway(
               new Job(SendType.External, "Localhost",
                  new List<Job>
                  {
                     ...
                     new Job(SendType.SelfToUserInterface, "REGL_DCMT_F", 10 /* Execute Actn_CalF_P */){Input = new XElement("Regulation", new XElement("Request_Requester", new XAttribute("rqtpcode", "003")))}
                  })
               );
         }
      }
