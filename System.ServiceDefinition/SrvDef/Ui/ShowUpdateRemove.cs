...
{
   public partial class ShowUpdateRemove : UserControl
   {
      public ShowUpdateRemove()
      {
         InitializeComponent();
      }
      ...
      private void sb_save_Click(object sender, EventArgs e)
      {
         ...
         Func<string> grphdrXmlData = new Func<string>(
            ...
            {
               string xmldata = "";
               ccbe_f_groupheaders.Properties.GetItems()
                  .OfType<CheckedListBoxItem>()
                  .Where(grp => grp.CheckState == CheckState.Checked).ToList()
                  .ForEach(grp => xmldata += string.Format("<GhID>{0}</GhID>", grp.Value));
               return xmldata;
            });
                  ...
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        new Job(SendType.Self, 07 /* Execute DoWork4AccessPrivilege */)
                        {
                           ...
                           {
                              string.Format("<Privilege>{0}</Privilege><Sub_Sys>1</Sub_Sys>", Convert.ToInt16(sb_save.Tag) == 1 ? 13 : 27),
                              "DataGuard"
                           },
                        ...
                        new Job(SendType.Self, 04 /* Execute DoWork4Odbc*/)
                        {
                           Input = new List<object>
                           {
                              ...
                              "xml",
                              XmlData(),
                              "{ Call ServiceDef.UpdateServiceOrGrpSrv(?) }",
                              "iProject",
                              "scott"
                           }
                        }
