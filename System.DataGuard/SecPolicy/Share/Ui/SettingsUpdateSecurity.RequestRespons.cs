...
{
   partial class SettingsUpdateSecurity : ISendRequest
   {
               ...
               new List<Job>
               {
                  new Job(SendType.External, "Commons",
                     new List<Job>
                     {
                        ...
                        {
                           Input = new List<string> {"<Privilege>37</Privilege><Sub_Sys>0</Sub_Sys>", "DataGuard"},
                           AfterChangedOutput = new Action<object>((output) => {
                              if ((bool)output)
                                 return;
                              #region Show Error
                              job.Status = StatusType.Failed;
                              MessageBox.Show(this, "خطا - عدم دسترسی به ردیف 37 امنیتی", "خطا دسترسی");
                              #endregion                           
                           })
                        },
                        #endregion                        
      ...
      {
         SwitchButtonsTabPage(VersionUpdate_Butn);
         job.Status = StatusType.Successful;
      }
   }
}
